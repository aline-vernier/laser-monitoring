...
class diagServer(threading.Thread):
    ...
    def _setup_signal(self) -> None:
        '''
        Use dictionary with server type? E.g. spectrometer, laser...etc.?
        .'''
        ...
        try:
            ...
            print(f"[diagServer {self.name}] Signal setup error:", e)

        if self.name == "LaserData":
            try:
                self._parent.signalLaserDataDict.connect(self._foo_bar)
            except Exception as e:
                print(f"[diagServer {self.name}] Signal setup error:", e)   
